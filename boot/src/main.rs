...
fn main() {
    ...
    for arg in args {
        if arg.as_str() == "--uefi" {
            use_uefi = true
        } else if arg.as_str() == "--no-run" {
            no_boot = true
        } else {
            panic!("unexpected argument `{}`", arg.as_str())
        }
    }

    let mode = create_disk_images(&kernel_binary_path, use_uefi);

    if no_boot {
        println!("Created disk image at `{}`", mode.display());
        return;
    }
    ...
    run_cmd
        .arg("-drive")
        .arg(format!("format=raw,file={}", mode.display()));

    if use_uefi {
        run_cmd.arg("-bios").arg("/usr/share/ovmf/x64/OVMF.fd");
    }
...
pub fn create_disk_images(kernel_binary_path: &Path, use_uefi: bool) -> PathBuf {
